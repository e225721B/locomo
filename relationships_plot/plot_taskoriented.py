#!/usr/bin/env python3
"""
TaskOriented（タスク志向）の関係値遷移を比較するグラフを出力するスクリプト
二つのディレクトリから relationship_reflection_details.jsonl を読み込み、
a_to_b と b_to_a の TaskOriented 値を折れ線グラフで表示します。
"""
...
def load_relationship_data(jsonl_path: str) -> dict:
    ...
    with open(jsonl_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            task_oriented = values.get('TaskOriented', 0)
            
            if direction == 'a_to_b':
                data['a_to_b'].append(task_oriented)
            elif direction == 'b_to_a':
                data['b_to_a'].append(task_oriented)
    
    return data


def plot_taskoriented_comparison(dir1: str, dir2: str, label1: str, label2: str, output_path: str = None):
    """2つのディレクトリの TaskOriented 値を比較するグラフを作成"""
    ...
    fig.suptitle('TaskOriented（タスク志向）の遷移比較', fontsize=16, fontweight='bold')
    ...
    ax1.set_xlabel('ターン')
    ax1.set_ylabel('TaskOriented (-3 〜 +3)')
    ...
    ax2.set_xlabel('ターン')
    ax2.set_ylabel('TaskOriented (-3 〜 +3)')
    ...
    ax3.set_xlabel('ターン')
    ax3.set_ylabel('TaskOriented (-3 〜 +3)')
    ...
    ax4.set_xlabel('ターン')
    ax4.set_ylabel('TaskOriented (-3 〜 +3)')
...
def main():
    parser = argparse.ArgumentParser(description='TaskOriented（タスク志向）の関係値遷移を比較するグラフを出力')
    ...
    plot_taskoriented_comparison(args.dir1, args.dir2, args.label1, args.label2, args.output)
